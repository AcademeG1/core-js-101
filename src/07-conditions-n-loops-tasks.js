...
function findFirstSingleChar(str) {
  const charFrequency = str.split('').reduce((f, char) => {
    if (f[char]) {
  ...
  const nonRepeatedChar = str.split('').find((char) => charFrequency[char] === 1);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startBracket}${lowerBound}, ${upperBound}${endBracket}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const sum = ccnArr.reduceRight((acc, digit, index) => {
    if (index % 2 === ccnArr.length % 2) {
      let doubled = digit * 2;
      if (doubled > 9) {
        doubled -= 9;
      }
...
function getDigitalRoot(num) {
  ...
  while (digits.length > 1) {
    digits = digits.reduce((sum, digit) => sum + digit, 0);
